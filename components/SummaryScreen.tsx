...
export default function SummaryScreen({ onRestart }: Props) {
    ...
    const padPrevRef = useRef({ confirm: false });
    ...
    // Bucle de Gamepad para el botón "X" (Confirmar = 14)
    useEffect(() => {
        ...
        const pollGamepad = () => {
            ...
            if (pad) {
                const isConfirm = pad.buttons[14]?.pressed;

                if (isConfirm && !padPrevRef.current.confirm) {
                    onRestart();
                }
                padPrevRef.current = { confirm: !!isConfirm };
            }
    ...
    return (
        <div className={styles.backdrop}>
            <div className={styles.container}>
                ...
                <button className={`${styles.btn} ${styles.btnFocused}`} onClick={onRestart}>
                    Volver al menú (Presiona X)
                </button>
            </div>
        </div>
    );
